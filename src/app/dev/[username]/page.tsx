...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  const title = `@${dev.github_login} - Sol City | ${contribs.toLocaleString()} contributions`;
  const description = `See @${dev.github_login}'s building in Sol City. ${contribs.toLocaleString()} contributions, ${dev.public_repos.toLocaleString()} repos, ${dev.total_stars.toLocaleString()} stars. Rank #${dev.rank ?? "?"} in the city.`;
...
export default async function DevPage({ params }: Props) {
  ...
  const breadcrumbJsonLd = {
    ...
    "@type": "BreadcrumbList",
    itemListElement: [
      { "@type": "ListItem", position: 1, name: "Sol City", item: baseUrl },
      {
        "@type": "ListItem",
        position: 2,
